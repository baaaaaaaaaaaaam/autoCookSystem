...
class systemSTAT:
    ...
    task_pc: int = None # 태스크의 프로그램 카운터 위치
    task_repeat: int = None # 태스크의 목표 반복 횟수
    task_run_id: int = None # 현재 수행 중 태스크의 액션 번호
    task_run_num: int = None # 태스크의 현재 반복 횟수
    task_run_time: float = None # 태스크 수행 시간(리셋 안됨)
    task_state: int = None # 태스크 상태(1: Idle, 2: Pause, 3: Run)
    default_speed: float = None # 로봇 동작의 기본 속도 배율(0 ~ 1)
    robot_state: int = None # 로봇 동작 상태(1: 정지, 3: 동작중)
    power_state: int = None # 파워상태(0: 48V 입력, 1: 48V 출력, 2: 24V 상태, 3: 비상스위치 상태, 4: PC스위치 상태, 5: 모터 제어기 상태)
    tcp_target: tuple = None 
    jnt_info: tuple = None 
    collision_detect_onoff: int = None # 충돌감지 설정 상태
    is_freedrive_mode: int = None # 직접교시 상태
    program_mode: int = None # 로봇 동작 모드
    ...
    op_stat_collision_occur: int = None # 외부 충돌 감지 상태(1: on / 0 : off)
    op_stat_sos_flag: int = None
    op_stat_self_collision: int = None # 자가 충돌 감지 상태(1: on / 0 : off)
    op_stat_soft_estop_occur: int = None # 로봇 일시 정지 상태(1: on / 0 : off)
    op_stat_ems_flag: int = None # 로봇 동작 오류 상태
...
def GetCurCollisionState():
    collisionState = systemstat_global.op_stat_collision_occur
    selfCollisionState = systemstat_global.op_stat_self_collision 
    return collisionState, selfCollisionState

def GetCurTaskState():
    taskState = systemstat_global.task_state
    if taskState == 1:
        return TASKSTATE.IDLE
    elif taskState == 2:
        return TASKSTATE.PAUSED
    elif taskState == 3:
        return TASKSTATE.RUN
...
def TaskLoad(program_name):
    msg = None
    msg = 'task load ' + program_name
    return SendCOMMAND(msg, CMD_TYPE.NONMOVE)
...
def MoveL(pnt, spd, acc):
    ...
    while True:
        print(systemstat_global.robot_state)
        if IsIdle():
            break
...
def ToolDigitalIn():
    return systemstat_global.tfb_digital_in
...
def ToolDigitalOut(port = int, voltage = int, type = DOUT_SET):
    dstatus = 0
    if type == DOUT_SET.LOW:
        dstatus = 0
    elif type == DOUT_SET.HIGH:
        ...
        dstatus = -1
    if port == 0:
        msg = 'tool_out ' + str(voltage) + ',' +  str(dstatus) + ',' + str(-1)
    elif port == 1:
        msg = 'tool_out ' + str(voltage) + ',' +  str(-1) + ',' + str(dstatus)
    return SendCOMMAND(msg, CMD_TYPE.NONMOVE)
